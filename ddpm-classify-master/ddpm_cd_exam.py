...
if __name__ == "__main__":
    ...
    parser.add_argument("--work_path", default="./train_result13/", help='工作日志保存位置')
    ...
    for feature_number in range(3, 6):
            ...
            for t in opt['model_cd']['t']:
                ...
                for current_epoch in range(start_epoch, n_epoch):
                    ...
                    for current_step, train_data in enumerate(train_loader):
                        ...
                        label = label.long()
                        # print("label.dtype_after:", label.dtype)
                        # f_A = []
                        # f_B = []
                        # for t in opt['model_cd']['t']:
                        ...
                        # print(t, type(fe_A_t))
                        #type(fe_A_t) is list
                        #for feature in fe_A_t: feature is list
                        #编码阶段特征图shape:(128,32,32),(256,16,16),()
                        # print("t:", t, "len(fe_A_t):", len(fe_A_t))#15
                        feature_et_batch = []
                        feature_dt_batch = []
                        ...
                        # for num, feature in enumerate(feature_et_batch):#len(feature_t_batchsize) is 15
                        #     channels = torch.unbind(feature, dim=1)
                        #     save_dir = "exam" + "/{}".format(t) + "/encode" + "/{}".format(num)
                        #     os.makedirs(save_dir, exist_ok=True)
                        #     for i, channel in enumerate(channels):
                        #         plt.imshow(channel[0].cpu().numpy(), cmap='viridis')  # 使用viridis colormap可视化
                        #         plt.title(f'Channel {i + 1}')
                        #         plt.savefig(os.path.join(save_dir, f'channel_{i + 1}.png'))
                        #         plt.clf()  # 清空图形，准备下一个通道的可视化
                    ...
                    #     for num, feature in enumerate(feature_dt_batch):#len(feature_t_batchsize) is 15
                    #         channels = torch.unbind(feature, dim=1)
                    #         save_dir = "exam" + "/{}".format(t) + "/decode" + "/{}".format(num)
                    #         os.makedirs(save_dir, exist_ok=True)
                    #         for i, channel in enumerate(channels):
                    #             plt.imshow(channel[0].cpu().numpy(), cmap='viridis')  # 使用viridis colormap可视化
                    #             plt.title(f'Channel {i + 1}')
        ...
        # break
                    ...
                    #     feature_x = torch.cat([feature_dt_batch[0], feature_dt_batch[1], feature_dt_batch[2]], dim=1)
                    #     feature_x1 = torch.cat([feature_dt_batch[3], feature_dt_batch[4], feature_dt_batch[5]], dim=1)
                    #     feature_x2 = torch.cat([feature_dt_batch[6], feature_dt_batch[7], feature_dt_batch[8]], dim=1)
                    #     feature_x3 = torch.cat([feature_dt_batch[9], feature_dt_batch[10], feature_dt_batch[11]], dim=1)
                    #     feature_x4 = torch.cat([feature_dt_batch[12], feature_dt_batch[13], feature_dt_batch[14]], dim=1)#2*2特征
                        # 相同尺寸的特征图叠加
                        pred_mask = model(image, feature_dt_batch[feature_number])
                        # print("pred_mask:", pred_mask.dtype, "label:", label.dtype)
                        train_loss = loss_func(pred_mask, label).to(device)
                        # 反向传播
                    ...
                    with torch.no_grad():
                        for current_step_val, val_data in enumerate(val_loader):
                            ...
                            # feature_x3 = torch.cat([feature_dt_batch[9], feature_dt_batch[10], feature_dt_batch[11]], dim=1)
                            # feature_x4 = torch.cat([feature_dt_batch[12], feature_dt_batch[13], feature_dt_batch[14]], dim=1) # 2*2特征
                            pred_mask = model(image_val, feature_dt_batch[feature_number])
                            # print(out)
