...
class TransFusion(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
                 embed_dim=96, Fusion_depths=[2, 2], Fusion_num_heads=[6, 6],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
...
if __name__ == '__main__':
    ...
    model = TransFusion(upscale=2, img_size=(256, 256),
    ...
    #
    # # # 假设你的模型是一个名为 TransFusion 的神经网络模型实例
    # # model = TransFusion()
    #
    # # 定义两个输入维度，例如 input_channels1, input_height1, input_width1 和 input_channels2, input_height2, input_width2
    # input_size1 = (30, 256, 256)
    # input_size2 = (30, 256, 256)
    #
    # input_data1 = torch.randn(1, *input_size1)
    # input_data2 = torch.randn(1, *input_size2)
    # # 打印模型结构摘要
    # summary(model, input_data1, input_data2)

    # print(height, width, model.flops() / 1e9)

    # x = torch.randn((1, 30, 256, 256))#通道数与embed_dim相对应
    # x = model(x, x)
    # print(x.shape)