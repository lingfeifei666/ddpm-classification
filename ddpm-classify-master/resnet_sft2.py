...
class ResNet(nn.Module):
    def __init__(self, num_classes: int, zero_init_residual: bool = False) -> None:
        ...
        # self.convd1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        # self.convd2 = nn.Conv2d(64, 64, kernel_size=7, stride=2, padding=3, bias=False)
        # self.convd3 = nn.Conv2d(64, 64, kernel_size=7, stride=2, padding=3, bias=False)
        # self.convd4 = nn.Conv2d(64, 64, kernel_size=7, stride=2, padding=3, bias=False)

        self.convd1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.convd2 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.convd3 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.convd4 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        self.layer3 = Basiclayer(128, 256, 64, stride=2)
        self.layer4 = Basiclayer(256, 512, 64, stride=2)
    ...
    def forward(self, x, x1):#以扩散模型特征为主特征，原图特征分别下采样，原始sft模块采用下采样操作
        #先做7x7的卷积
        x  = self.conv1(x)
        x2 = self.convd1(x1)
        ...
        x = self.fc(x)
        return x
    # def forward(self, x, x1):#以扩散模型特征为主特征，原图特征逐层下采样，原始sft模块不采用下采样操作
    #     # 先做7x7的卷积
    #     x = self.conv1(x)
    #     x2 = self.convd1(x1)
    #     # print(x.shape, x2.shape)
